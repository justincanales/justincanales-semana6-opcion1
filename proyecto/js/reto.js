...
listaPlatillos.forEach(function(Miplato){

   let  divimag=document.createElement("div");

   divimag.setAttribute("class","imagen");

   divimag.innerHTML=`<img src=${Miplato.imagen}></img>`;

   contenido.appendChild(divimag);

   let divtexto=document.createElement("div");
   divtexto.innerHTML=`<h4>${Miplato.nombre}</h4>
                              <p>${Miplato.descripcion}</p>`;
  
        let divprecio=document.createElement("div");
        divprecio.setAttribute("class", "precio");
        divprecio.innerHTML=`<span>S/. ${Miplato.precio}</span>`
